...
export const generateHourArray = (defaultTz: string, timezone: string, isPrimary = false) => {
  ...
  if (!isPrimary && tz && defTz) {
    ...
    if (timeDifference > 0) {
      return [...initial.slice(roundedTimeDiff, initial.length), ...initial.slice(0, roundedTimeDiff)];
    } else {
      return [...initial.slice(initial.length - roundedTimeDiff, initial.length), ...initial.slice(0, initial.length - roundedTimeDiff)];
    }
  }
...
export const getMetaData = (defaultTz: string, timezone: string, isPrimary = false) => {
  const tz = Timezones.find((e) => e.value === timezone);
  const defTz = Timezones.find((e) => e.value === defaultTz);
